const ws = require('ws'),
      ...
      { platform } = require('os'),
      APIBaseURL = 'https://discord.com/api/v8';
...
class Client extends EventEmitter {
  /**
   * @param {ClientOptions} [options={ intents: 32767 }] The options for the client.
   */
    ...
    if (path !== undefined) {
      for (const file of readdirSync(path).filter(f => /\.[jt]s$/.test(f))) {
        try {
          ...
          console.error(`Command ${file.split('.')[0]} failed to load, error:\n${err}`);
        }
      }
    }
  }
  ...
  connect(token = this.token) {
    ...
    this.socket.once('open', () => {
      ...
      this.socket.on('message', message => this.emit('debug', message));
