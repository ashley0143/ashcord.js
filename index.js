...
class Client extends EventEmitter {
   ...
   */
  connect(token) {
    if (typeof token !== 'string' && typeof this.token !== 'string')
    ...
    this.socket.once('open', () => {
      ...
      this.socket.on('message', (message, flag) => websocket(this, message, flag));
      this.socket.on('close', c => {
        ...
        switch (c) {
          ...
          case 4013:
            throw new Error('You sent an invalid intent for a Gateway Intent. You may have incorrectly calculated the bitwise value. or AMOGUS uwu nyaaa'); // I'm not ok
          case 4014:
